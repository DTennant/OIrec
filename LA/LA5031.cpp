...
#include <vector>
using namespace std;
const int MAX_C = 500100;
struct Command{
	char type;
	int x,p;
}commands[MAX_C];
const int MAX_N = 20010;
const int MAX_M = 60010;
int n,m,weight[MAX_N],from[MAX_M],to[MAX_M],removed[MAX_M];
int pa[MAX_N];
int findset(int x){return pa[x] == x ? pa[x] : pa[x] = findset(pa[x]);}
Node* root[MAX_N];
...
int query_cnt;long long query_tot;
